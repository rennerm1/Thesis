...
class O_LRP:
    ...
    def C_cfg(self, Customer_N, mean_demand, max_demand, min_demand, seed_customers, seed_demand):
        ...
        # Load Nodes (Output from OSMNX)
        nodes_df = pd.read_hdf("nodes_df_wue", "nodes_df")
        

        # Only work with Nodes that have been used in HERE API Output
    ...
    def DC_cfg(self, DC_N, customer_N, fixed_cost_factor, avg_basket_size, capacity_DC=[], varCost_DC = []):
        ...
        #Locations and OsmidIDs
        lat_DC =[49.7582904, 49.8610025,49.8743879, 49.7726213, 49.7109099]
        lon_DC=[9.8039055, 9.8037194, 10.032524, 10.0631658, 10.0201193]
        osmid =[283531080, 1312952369, 258899345, 28338982, 41891309]
    ...
    def solve(self,Gap, Time_Limit, Para, DC_Max, Drive_Time_Max):
        ...
        # If we should ever want to open a specific number of ODCs
        if DC_Max == 0:
            self.m.update()
        elif DC_Max == 1:
            self.m.addConstr(quicksum(self.y[i] for i in self.I) == 1)
    ...
    # Set up Printers for information at glance    
    def DC_Print(self):
    ...
    def Basic_Plot(self):
        G = ox.graph_from_place("Wuerzburg, Germany", buffer_dist= 10000, network_type = "drive")
